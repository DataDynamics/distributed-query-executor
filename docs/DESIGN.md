...
`template.dir`(기본 `/data1/distributed-query-executor/templates`, 개발 시 `templates`) 아래 **`<template_id>/` 디렉터리 하나가 하나의 이관 시나리오**입니다.
...
- **소스 실행 = 커스텀 함수 위임(`/query-run`)**: query-execute 의 소스 실행은 executor 가 소스(Trino 등)를 **직접 접속하지 않고**, 설정으로 지정한 외부 Python 함수에 위임합니다. executor 의 `POST /query-run` 이 `query.func.module`(dotted path, `importlib` 로딩·캐시)로 함수를 찾아 `run(sql, config=<query.func.config.* dict>, limit)` 를 호출하고, 반환된 `QueryResult`(또는 동일 키 dict)를 그대로 응답합니다. **설정은 config.properties 에서 자유 정의** — `query.func.config.<키>=<값>` 을 프리픽스로 모아(코드/`config.yml` 수정 없이) 함수에 dict 로 넘깁니다(값은 문자열, 형변환은 함수 책임; `src/core/config.py` 의 `_collect_prefix`). 미설정 시 400, 로드/실행 실패 시 502. 참조 구현: `customs/query_funcs/trino_runner.py`. (임의 SQL 미리보기 `/datasources/{name}/query`(§B 운영 점검용)와 이관 `_source_connect` 의 소스 접속은 별개로 built-in 유지.)
...
- **템플릿 계약**: SELECT 조각은 `WHERE {{ task_column | sql_ident }} = {{ task_date | sql_str }}` 처럼 하루치를 조회하고, INSERT/staging 은 날짜를 참조하지 않습니다. 예제: `templates/daily_sales/`.
