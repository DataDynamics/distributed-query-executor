...
여기서도 앞서 말한 원칙이 똑같이 적용됩니다. 메트릭을 담는 `executor_health_metrics` 테이블 역시 앱이 알아서 만들어 주지 않으므로, 통합 스키마인 `config/postgresql.sql` 을 **먼저 적용**해야 합니다. 만약 `monitor.db_dsn` 이 다른 데이터베이스를 가리킨다면 그 데이터베이스에도 동일하게 스키마를 적용해 주어야 합니다.
