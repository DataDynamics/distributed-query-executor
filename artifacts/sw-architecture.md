...
실패했는지를 언제든 볼 수 있게 하고, 실패한 조각만 골라 다시 돌릴 수 있게 한다. 셋째로 **수용
통제를 입구에 두어** 소스와 대상이 감당할 수 있는 만큼만 받아들인다.

여기에 운영 환경의 제약이 더해졌다. 외부 네트워크가 막힌 에어갭이고, 운영체제가 주는 파이썬을
그대로 써야 하며, 적재 대상이 MPP 라 데이터를 마스터 한 곳으로 밀어 넣으면 그 한 노드가 천장이
된다. 이 제약들이 아키텍처의 모양을 상당 부분 결정했다.
...
| 구성요소 | 역할 | 수량 | 비고 |
|---|---|---|---|
| coordinator | 요청 접수, 검증과 분할, 수용 통제, 배분, 상태 집계 | 1대(가용성이 필요하면 N대) | 상태를 메모리에 두므로 단일 워커 |
...
`202` 는 접수했다는 뜻일 뿐이므로 종료 상태를 확인해야 하고, 온전한 성공은 `DONE` 하나뿐이다.
다른 하나는 **재실행 안전성**이다. 타임아웃 뒤에 그냥 다시 보내면 같은 작업이 두 번 돌 수 있으므로
멱등 키를 실어 보내고, 적재가 두 번 되면 곤란한 업무는 파티션을 지우고 넣는 실행 방식을 고른다.

### 1.2 운영·관제 업무 관점

운영자는 세 가지를 본다. 첫째로 **살아 있는지**를 본다. 클러스터 통합 상태를 한 번 조회하면
executor 가 다 붙어 있는지, 작업이 쌓이고 있는지, 자원이 남아 있는지가 함께 나온다. 브라우저를 쓸
수 있으면 대시보드로, 터미널만 있으면 같은 API 를 읽는 모니터로 본다.

둘째로 **실패를 좇는다**. 사용자가 작업 식별자를 들고 오면 그 하나로 상태와 실패한 task 를 확인하고,
같은 식별자로 로그를 모은다. 모든 로그 줄에 작업과 task 식별자가 붙고 실행한 SQL 이 함께 남으므로
...
| 유형 | 책임 | 적용 구성요소 | 대표 기술 |
|---|---|---|---|
| API·제어 | 요청을 받아 검증·분할하고 수용을 판단해 배분하며 상태를 집계한다 | coordinator | FastAPI · asyncio · sqlglot · Jinja2 |
...
개발은 실제 DB 없이 한다. 소스와 대상이 없으면 백엔드가 아무것도 읽고 쓰지 않는 대역으로 뜨고,
테스트는 그 대역과 가짜 실행기로 전 구간을 돈다. 그래서 노트북에서도 검증·분할·수용·상태 전이·
대시보드·로깅까지 확인할 수 있다.
...
두 가지가 개발을 특히 편하게 한다. **local 실행 방식**은 executor 프로세스를 따로 띄우지 않고
coordinator 안에서 백엔드를 직접 실행하므로 설정이나 쿼리가 의도대로 도는지만 볼 때 좋고,
**템플릿 자동 재적재**는 `.sql.j2` 를 고칠 때마다 서비스를 재기동하지 않게 해 준다.
...
| 적재 대상이 MPP | 마스터로 밀어 넣으면 한 노드가 천장이 된다 | 파일을 만들어 세그먼트가 당겨 읽는 2단계 경로를 둔다 |
...
**API·제어 SW 기술 구조**는 요청을 받아 판단하는 쪽이다. 검증과 분할, 수용 통제, 배분, 상태 집계를
맡고 데이터에는 손대지 않는다. 판단이 빨라야 하므로 비동기로 돌고, 무거운 일은 모두 아래 유형에
넘긴다.

**실행·적재 SW 기술 구조**는 데이터를 실제로 옮기는 쪽이다. 소스에서 배치 단위로 읽어 대상에 넣고
그 진행을 보고한다. 블로킹 드라이버를 쓰므로 스레드로 넘겨 이벤트 루프를 막지 않으며, 실행 방식에
따라 여러 경로를 갖되 바깥에서 보는 계약은 하나로 맞춘다.
...
모듈은 요청이 지나가는 순서대로 나뉜다. 라우팅과 멱등 처리, 날짜 fan-out 은 앱 계층이 맡고,
SQL 검증은 파서가, 분할은 splitter 가, 서버 측 쿼리 생성은 템플릿 엔진이 맡는다. 그 뒤로 수용
통제와 배분이 dispatcher 에 모여 있고, 상태 저장과 이력, 헬스 모니터링, executor 선택은 각자
모듈로 떨어져 있다. 화면(대시보드·모니터)은 API 를 읽기만 하므로 제어 흐름에 끼어들지 않는다.

#### 2.1.2 런타임 구성 관점

한 프로세스가 단일 워커로 돈다. 상태를 프로세스 메모리에 들고 있어 워커를 늘리면 같은 작업을 두
곳이 서로 다르게 알게 되기 때문이다. 그래서 확장은 워커가 아니라 인스턴스 수로 한다.

요청 처리에는 **동기 구간과 비동기 구간의 경계**가 뚜렷하다. 검증과 분할, 수용 판단까지는 요청
핸들러 안에서 끝내 잘못된 요청을 즉시 되돌려 주고, 실제 실행은 응답을 보낸 뒤 백그라운드로
넘어간다. 백그라운드에서는 실행 슬롯을 기다렸다가 task 를 병렬로 띄우고 상태를 폴링한다. 블로킹
DB 호출은 스레드로 넘기되 로그 문맥을 함께 들려 보내, 그 스레드가 남기는 줄에도 작업 식별자가
붙게 한다.
...
요청이 들어오면 멱등 키를 먼저 확인하고, 템플릿 요청이면 SQL 을 렌더한 뒤 파서로 검증하고
파티션 값 목록을 N 등분해 task 를 만든다. 수용 통제를 통과하면 작업을 만들면서 멱등 키를 원자적으로
선점하고 `202` 를 돌려준다. 그 뒤 백그라운드가 실행 슬롯을 잡아 상태를 RUNNING 으로 올리고 task 를
배분하며, 모두 끝나면 종료 상태를 집계한다.
...
| 수용 통제 | 실행 슬롯과 대기 큐 관리, 초과 시 거절 | `try_admit()` · `slot()` |
| dispatcher | 수명주기와 배분, 폴링, 취소, 2단계 적재의 Phase 2 | `run()` · `_execute()` |
| 저장소 | 작업 상태 보관과 멱등 키 선점 | `claim_and_add()` · `save()` · `get()` |
| 이력·모니터 | 상태 전이 기록, executor 헬스와 메트릭 수집 | `record()` · `snapshot()` |

#### 2.2.3 구조 상세 설계

**수용 통제는 세 층으로 겹친다.** 입구에서 동시 실행 작업 수와 대기 큐를 제한하고, 그 아래에서
...
**멱등은 두 층위로 나뉜다.** 요청 멱등은 같은 키에 같은 본문이면 기존 작업을 재생하고 본문이 다르면
거절하는 것이고, 데이터 멱등은 적재 전에 담당 파티션을 지우고 넣어 재실행해도 중복되지 않게 하는
것이다. 앞의 것은 저장소의 원자적 선점으로, 뒤의 것은 실행 방식이 지원할 때만 성립한다.
...
대상에 넣고 정리하는 일은 coordinator 가 배리어 뒤에 한 번만 수행한다.
...
| 모듈 | 하는 일 |
|---|---|
| 설정 로더·설정 | properties 값으로 YAML 자리표시자를 채워 설정 객체 하나로 굳힌다 |
| 로깅 | 일 단위 롤링, 작업·task 식별자 자동 주입, 경고 전용 로그 분리 |
...
맡는다. 백엔드는 규약으로 정의돼 있어 실제 구현과 시험용 대역이 같은 자리에 들어간다. 그 밖에
오브젝트 스토리지 클라이언트, 자기 상태 보고, task 이력 기록이 각각 떨어져 있다.

![클래스 — executor 와 적재 백엔드](images/class-executor-backend.svg)

#### 3.1.2 런타임 구성 관점

동시에 실행하는 task 수를 세마포어로 제한하고, 대상 연결은 풀로 재사용하되 반납할 때 세션을
초기화해 앞 task 가 만든 임시 객체가 다음 task 와 부딪히지 않게 한다. 블로킹 드라이버 호출은 모두
스레드로 넘기며, 읽기와 쓰기를 별도 스레드로 겹쳐 실행해 벽시계를 줄인다. 종료 신호를 받으면
진행 중인 task 가 끝나기를 정해진 시간만큼 기다렸다 내려간다.

#### 3.1.3 배포 관점

한 호스트에 포트를 달리해 여러 인스턴스를 띄울 수 있고, 처리량은 이 수로 늘린다. coordinator 의
목록에 없으면 프로세스를 띄워도 일이 가지 않으므로 목록을 먼저 고치고 띄운다. 세그먼트 로컬 파일을
쓰는 실행 방식은 executor 를 대상 세그먼트와 같은 호스트에 두어야 하고, 여러 coordinator 를 쓸
때는 자기 상태를 공유 저장소에 직접 보고하게 켠다.
...
| 백엔드 규약 | 실행 방식별 진입점을 하나의 계약으로 묶는다 |
| 실제 백엔드 | 소스 커서 열기, 스트리밍, 대상 적재와 스테이징 SQL 실행 |
| 대역 백엔드 | 대상 접속 정보가 없을 때 뜨는 시험용 구현 |
...
**실행 방식이 데이터 경로를 정한다.** 같은 요청 스키마에서 필드 하나로 경로가 갈리며, 곧장 넣을지
...
파일을 대상이 읽게 만드는 일은 coordinator 가 배리어 뒤에 한다.
...
제어 쪽과 같은 설정·로깅·마스킹·시각·메트릭 모듈을 그대로 쓴다. 여기에 더해 단계 기록 모듈이
task 의 타임라인을 만들고, 스테이징 SQL 조립 모듈이 외부테이블 정의와 적재·정리 문장을 순수 함수로
만들어 준다. 순수 함수로 둔 이유는 실제 DB 없이 문자열만 비교해 시험하기 위해서다.

#### 3.3.2 솔루션 관련 공통 모듈

소스 접속은 impyla 와 SASL 패키지, 대상 접속은 psycopg, 오브젝트 스토리지는 boto3 를 쓴다. 셋 다
필요한 실행 방식에서만 불러오므로, 쓰지 않는 기능의 패키지가 없어도 나머지는 동작한다.
...
분할은 순수 함수로 떨어져 있어 실제 DB 없이 시험된다. 수명주기는 기반 클래스가 모두 쥐고 하위
클래스는 실행 한 가지만 구현하는데, 그래서 원격 실행과 in-process 실행이 같은 상태 전이와 같은
집계 규칙을 따른다.

![클래스 — coordinator 요청 처리](images/class-coordinator.svg)

확장이 필요한 자리도 구조 안에 있다. 쿼리는 템플릿으로 바깥에 두고, 템플릿에서 쓸 함수는 등록
방식으로 더하며, 저장소는 규약 하나에 구현이 셋이라 단일 노드와 다중 노드가 코드 변경 없이 갈린다.

#### 1.1.2 실행·적재 SW 기술 구조 관점

executor 는 상태머신과 동시 실행 상한만 직접 들고, 나머지는 백엔드 규약 뒤에 있다. 그래서 대상
접속 정보가 없으면 대역 구현으로 떠서 전 구간 시험이 가능하고, 소스가 커서를 주지 않아도 어댑터가
그 자리를 메운다. 실행 방식이 늘어나도 앱 계층은 진입점 하나를 더 부를 뿐이다.
...
여러 대를 띄우려면 상태를 공유 저장소로 외부화하고 각자 생존을 알리게 켠다. 이때 입구 한도가
인스턴스마다 따로 적용된다는 점을 감안해 총량을 나눠 잡는다.
...
늘린 뒤에는 함께 움직여야 하는 값이 있다. 디스패치 상한이 늘어난 플릿 용량보다 작지 않은지, 대상이
허용하는 세션 수를 넘지 않는지 확인한다. 세그먼트 로컬 파일을 쓰는 실행 방식이라면 새 노드도
세그먼트 호스트 위에 있어야 하고 호스트명 설정이 정확히 맞아야 한다.
...
시험은 실제 DB 없이 돈다. 대역 백엔드와 가짜 실행기로 검증·분할·수용·상태 전이·대시보드·로깅까지
덮으며, 터미널 화면은 화면 밖에 쓰면 실패하는 가짜 curses 로 여러 화면 크기에서 훑는다.
...
주의할 구현 규약이 하나 있다. 미리보기 함수와 이관 함수는 이름이 비슷해도 계약이 다르다. 상한을
두고 자르는 쪽과 전량을 돌려주는 쪽을 섞으면 잘린 결과가 조용히 적재되므로, 판정은 한 곳에서만
하고 설정이 없으면 폴백 없이 명확히 실패시킨다.
...
그리기와 떼어 두어 시험할 수 있게 했고, 에어갭을 고려해 표준 라이브러리만 쓴다.
...
| 메타 저장소 | 작업 상태, 실행 이력, executor 상태, 예약, 메트릭 | 읽기·쓰기 | coordinator · executor |
| 요청 주체 | 작업 제출과 상태·결과 조회 | 수신 | coordinator |
...
| HTTP 호출 | coordinator 와 executor 사이 | 작업 지시와 상태 조회. 실패하면 재시도 후 다른 노드로 넘긴다 |
| 외부테이블 당겨 읽기 | 2단계 적재의 Phase 2 | 파일을 만들어 두고 대상 세그먼트가 병렬로 읽는다 |
| 오브젝트 스토리지 경유 | 세그먼트와 같은 호스트에 둘 수 없을 때 | 위치에 무관해 배치 제약이 없다 |

#### 3.3.2 개발환경

연계 대상이 없어도 개발과 시험이 된다. 대상 접속 정보가 비면 아무것도 읽고 쓰지 않는 대역이 뜨고,
스테이징 SQL 조립은 순수 함수라 문자열만 비교하면 되며, HTTP 연계는 가짜 실행기로 대신한다. 실제
연계를 처음 붙일 때는 작은 작업 하나를 끝까지 돌려 보는 것이 가장 확실한 확인이다.
...
따라 다르게 정해 두었다. HTTP 는 재시도 뒤 다른 노드로 넘기고, DB 는 실패를 그대로 올려 작업 상태로
