...
같은 내용을 엑셀로도 두었다. `tables.xlsx` 는 테이블마다 시트를 하나씩 두고 이 문서와 같은 열을
담으며, 맨 앞의 개요 시트에서 테이블 목록과 컬럼 수를 볼 수 있다. 검토 의견을 표에 직접 달거나
사내 표준 양식에 옮겨 붙일 때 쓴다. 값을 고쳤다면 이 문서와 엑셀을 함께 갱신한다.
