...
def masked_config(settings) -> list[dict]:
    ...
    rows: list[tuple[str, str, object, str]] = [
        ...
        ("coordinator", "stage_unique_staging",
         getattr(settings, "stage_unique_staging", True),
         "stage_insert: staging 테이블명에 task_id 를 붙여 고유화(풀 재사용 시 TEMP 충돌 방지)"),
        ...
        ("stage", "csv_delimiter", getattr(settings, "stage_csv_delimiter", "`"),
         "CSV 컬럼 구분자. executor write 와 외부테이블 FORMAT 이 공유(s3_stage 도 동일)"),
        ...
        ("stage", "cleanup", getattr(settings, "stage_cleanup", True),
         "Phase 3 에서 로컬 CSV 디렉터리/외부테이블을 정리할지 여부"),
        ("stage", "validate_hosts", getattr(settings, "stage_validate_hosts", True),
         "Phase 2 전 file:// 호스트를 gp_segment_configuration 과 대조 검증(불일치 조기 실패)"),
        ("stage", "max_files_per_host", getattr(settings, "stage_max_files_per_host", 0),
         "호스트당 최대 파일 수(0=호스트별 primary 세그먼트 수 S_h 를 조회해 사용)"),
        ...
        ("s3", "external_schema",
         ...
         "Phase 2 외부테이블 스키마 → <schema>.s3ext_<job_id>. 비우면 GP 세션 search_path 를 따름"),
        ("s3", "endpoint_url", getattr(settings, "s3_endpoint_url", "") or "(AWS 기본)",
         "온프렘 S3 호환(MinIO/Ceph) 엔드포인트. 업로드 경로에만 적용"),
        ...
        ("s3", "pxf_server", getattr(settings, "s3_pxf_server", "") or "(없음)",
         "GP 읽기용 PXF SERVER 이름($PXF_BASE/servers/<server>/s3-site.xml 에 자격증명)"),
        ("s3", "pxf_profile", getattr(settings, "s3_pxf_profile", "s3:csv"),
         "PXF 프로파일(기본 s3:csv)"),
        ("s3", "gp_location_template",
         getattr(settings, "s3_gp_location_template", "") or "(기본 pxf:// 형식)",
        ...
        ("template", "dir", getattr(settings, "template_dir", ""),
         "템플릿 루트 디렉터리(하위 <template_id>/manifest.yml + *.sql.j2)"),
        ("template", "auto_reload", getattr(settings, "template_auto_reload", False),
         "파일 변경 자동 리로드(개발 편의). 운영은 false 로 stat 비용 제거"),
        ("template", "func_modules",
         ", ".join(getattr(settings, "template_func_modules", []) or []) or "(없음)",
         "커스텀 필터/글로벌 등록 모듈(기동 시 import). 예: customs.query_funcs.my_funcs"),
        ("template", "validate_ddl_single_stmt",
         getattr(settings, "template_validate_ddl_single_stmt", True),
         "렌더된 DDL/INSERT 를 단일 SQL 문으로 강제(다중 문 인젝션 방지)"),
