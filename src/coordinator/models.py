...
class Job:
    ...
    # 이 job 의 SELECT 를 읽을 소스 엔진(impala 기본). impala 가 아니면 executor 가 설정
    # query.func.<name>.connect 의 커스텀 함수 연결로 읽는다. task 마다 그대로 실려 간다.
...
class CreateJobRequest(BaseModel):
    ...
    datasource: Optional[str] = Field(
        ...
        "그것도 없으면 서버 source.type(기본 impala). impala 외의 값은 executor 설정 "
        "query.func.<datasource>.connect 의 커스텀 접속 함수로 읽는다(예: trino). "
        "적재 대상(Greenplum)과 exec_mode 는 이 값과 무관하게 그대로다.",
    )
    # ── local_stage 전용 필드 ──
