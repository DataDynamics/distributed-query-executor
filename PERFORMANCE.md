...
[README.md](README.md) 와 [deploy/README.md](deploy/README.md) 를 함께 보면 좋습니다.

처음 시작하기 전에 한 가지 약속만 기억해 두면 됩니다. 이 시스템의 모든 파라미터는
`packaging/config/config.properties` 라는 파일에 자바 스타일의 `key=value` 형태로 적습니다.
...
> 스키마는 앱이 자동 생성하지 않는다. 기동 전에 `packaging/config/postgresql.sql`
