...
# ───────────────────── manifest datasource(실행 엔진 선택) ─────────────────────


def test_manifest_datasource_parsed_into_defaults(tmp_path):
    """manifest 의 datasource 가 스칼라 기본값으로 파싱된다(query-execute 가 읽는 자리)."""
    d = tmp_path / "ds"
    d.mkdir()
...
def test_jobs_allows_impala_datasource_template(tmpl_client):
    """이관 템플릿이 datasource 를 안 쓰거나 impala 면 그대로 통과한다(하위 호환)."""
    resp = tmpl_client.post("/jobs", json={
        "template_id": "sales_migration",
        "params": {"start_dt": "2026-01-01", "end_dt": "2026-01-04", "regions": ["KR"]},
        "dry_run": True,
